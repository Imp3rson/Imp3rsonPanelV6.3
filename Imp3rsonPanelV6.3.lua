-- ============================================
-- 🔪 Imp3rson MM2 Hub v3 - PARTE 1/3 (CORE)
-- ============================================
...
MM2.Services = { Players = Players, RunService = RunService, UIS = UIS,
    CoreGui = game:GetService("CoreGui"), RS = game:GetService("ReplicatedStorage") }
...
MM2.Settings = {
    ...
    AutoPickupGun = false, AutoNotifyWeapon = false, GunNotifyDistance = 200,
}
...
Players.PlayerRemoving:Connect(function(plr)
    if MM2.Cache[plr] then MM2.wipeDrawings(MM2.Cache[plr]) MM2.Cache[plr] = nil end
end)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if show then
            ...
            if MM2.DrawingOK then
                ...
                local topSp = cam:WorldToViewportPoint(root.Position + Vector3.new(0, 3, 0))
                local botSp = cam:WorldToViewportPoint(root.Position - Vector3.new(0, 3, 0))
...
RunService.RenderStepped:Connect(function()
    ...
    if t then
        ...
        if part then
            ...
            if Settings.AimRage then cam.CFrame = cf
            else cam.CFrame = cam.CFrame:Lerp(cf, math.clamp(Settings.AimSmooth, 0.01, 1)) end
        end
    end
end)
...
print("[✅] MM2 v3 PARTE 1/3 OK")

-- ============================================
-- 🔪 Imp3rson MM2 Hub v3 - PARTE 2/3 (FUNÇÕES)
-- ============================================

local MM2 = _G.Imp3rsonMM2
if not MM2 then warn("[❌] Rode a PARTE 1 primeiro!") return end
...
task.spawn(function()
    while not MM2.Unloaded do
        pcall(function()
            ...
            for part in pairs(droppedGuns) do
...
print("[✅] MM2 v3 PARTE 2/3 OK")

-- ============================================
-- 🔪 Imp3rson MM2 Hub v3 - PARTE 3/3 (UI)
-- ============================================

local MM2 = _G.Imp3rsonMM2
if not MM2 then warn("[❌] Rode as PARTES 1 e 2 primeiro!") return end
...
Header.Position = UDim2.new(0, 0, 0, 0); Header.Size = UDim2.new(1, 0, 0, 44); Header.ZIndex = 3

local Title = Instance.new("TextLabel")
Title.Parent = Header; Title.BackgroundTransparency = 1
Title.Position = UDim2.new(0, 16, 0, 0); Title.Size = UDim2.new(1, -140, 1, 0)
Title.Font = Enum.Font.GothamBold; Title.Text = "🔪 Imp3rson MM2 Hub  •  v3"
...
roleLbl.TextXAlignment = Enum.TextXAlignment.Right; roleLbl.ZIndex = 4
...
local function winBtn(text, xOffset)
    ...
    b.TextColor3 = Color3.fromRGB(255, 255, 255); b.TextSize = 14; b.ZIndex = 5
...
Divider.Position = UDim2.new(0, 0, 0, 44); Divider.Size = UDim2.new(1, 0, 0, 1); Divider.ZIndex = 3
...
addToggle(pages.Murder, "🔥 AIM RAGE", "AimRage", Settings.Colors.Murder)
...
print("[✅] MM2 v3 PARTE 3/3 OK — Hub completo")
